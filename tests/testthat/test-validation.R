test_that("constructors reject out-of-range parameters", {
  ...
  expect_error(dfr_kldist(window_size = "serie"), "single numeric value")
...
test_that("stealthy validates its strategy arguments", {
  expect_error(stealthy(model = NULL, drift_method = dfr_ddm(), class_balance = "nope"), NULL)
  expect_error(stealthy(model = NULL, drift_method = dfr_ddm(), obsolete_model = "nope"), NULL)
  expect_error(stealthy(model = NULL, drift_method = dfr_ddm(), th = 2), "between 0 and 1")
})

test_that("multi criteria validates its arguments", {
  expect_error(dfr_multi_criteria(drifter_list = list()), "non-empty list")
  expect_error(dfr_multi_criteria(drifter_list = list(bad = dfr_ddm())), "dist_based")
  expect_error(
