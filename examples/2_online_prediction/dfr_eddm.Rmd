...
model <- stealthy(
  model=cla_dtree(target, slevels), # Naive Bayes Classifier
  drift_method=dfr_eddm(), # Using EDDM
  norm_class=nrm_memory(norm_class = minmax()), # Normalization with MinMax
...
# Calculate metrics
beta <- 1
precision <- sum(results['tp'])/(sum(results['tp']) + sum(results['fp']))
recall <- sum(results['tp'])/(sum(results['tp']) + sum(results['fn']))
accuracy <- (sum(results['tp']) + sum(results['tn']))/(sum(results['tp']) + sum(results['fp']) + sum(results['tn']) + sum(results['fn']))
