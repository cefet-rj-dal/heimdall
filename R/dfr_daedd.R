#'@title Denoising Autoencoder-Based Drift Detection method
#'@description Denoising Autoencoder-Based method for concept drift detection <doi:0.1109/ICDMW58026.2022.00109>.
...
#DAEDD detection: Daniil Kaminskyi, Bin Li and Emmanuel Müller. “Reconstruction-based unsupervised drift detection over multivariate streaming data.” 2022 IEEE International Conference on Data Mining Workshops (ICDMW).
...
#'model <- stealthy(cla_nb(target, slevels), dfr_daedd(features=features, input_size=length(features), encoding_size=3, window_size=1800))
#'
...
#'
#'@export
dfr_daedd <- function(features, input_size, encoding_size, batch_size = 32, num_epochs = 1000, learning_rate = 0.001, window_size=100, monitoring_step=1700, criteria='mann_whitney') {
  ...
  state$autoencoder <- dae_encode_decode(input_size=input_size, encoding_size=encoding_size, batch_size=batch_size, num_epochs=num_epochs, learning_rate=learning_rate)
  state$is_fitted <- FALSE
  
  obj$drifted <- FALSE
  obj$state <- state
  class(obj) <- append("dfr_daedd", class(obj))
  return(obj)
}

#'@export
update_state.dfr_daedd <- function(obj, value){
  #print('Update State')
...
#'@export
fit.dfr_daedd <- function(obj, data, ...){
...
#'@export
reset_state.dfr_daedd <- function(obj) {
  obj$drifted <- FALSE
  obj$state <- dfr_daedd(
